        ...
        for att in element.attrib.keys():           #parsing attributes
            if att in CREATED:
                ...
                try:
                    ...
                    print element.attribfloat[att], 'not number!!!!!'
            elif (att == 'lat'):
                try:
                    pos[0] = float(element.attrib[att])
                except ValueError:
                    print element.attrib[att], 'not number!!!!!'
            else:
                ...
                if not problemchars.search(child.attrib['k']):
                    if child.attrib['k'].startswith('addr:'):
                        if child.attrib['k'] == 'addr:housenumber':
                            ...
                            address['city'] = child.attrib['v']
                        elif child.attrib['k'] == 'addr:postcode':
                            address['postcode'] = child.attrib['v']
                        elif child.attrib['k'] == 'addr:country':
                            address['country'] = child.attrib['v']
                        elif child.attrib['k'] == 'addr:state':
                            address['state'] = child.attrib['v']
                        #else:
                            #print 'addresses>>>>>', child.attrib['k'], child.attrib['v']
                    else:
